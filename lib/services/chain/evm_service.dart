...
class EVMService {
  ...
  final MyHttpClient myHttpClient = GetIt.I<MyHttpClient>();
  ...
  EVMService({required this.chainSupported}) {
    ...
    ethClient = Web3Client(chainMetadata.rpc, http.Client());
  ...
  Future<void> personalSign(String topic, dynamic parameters) async {
    ...
    if (await requestApproval(message)) {
      try {
        ...
        final keys = GetIt.I<KeyService>().getKeysForChain(
          chainSupported.chainId,
        );

        final signature = EthSigUtil.signPersonalMessage(
          privateKey: keys[0].keyPair.privateKey,
  ...
  Future<void> ethSignTransaction(String topic, dynamic parameters) async {
    ...
    if (result is Transaction) {
      try {
        ...
        final credentials =
            EthPrivateKey.fromHex('0x${keys[0].keyPair.privateKey}');
